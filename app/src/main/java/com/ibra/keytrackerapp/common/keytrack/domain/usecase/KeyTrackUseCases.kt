...
class KeyTrackUseCases @Inject constructor(
    ...
    suspend fun giveKey(token: String, userId: String, keyId: String) =
        keyTrackRepository.giveKey(token, userId, keyId)
    ...
    suspend fun getPeople(token: String, page: Int, name: String?) =
        keyTrackRepository.getPeople(token, page, name)
}